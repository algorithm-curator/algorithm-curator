...
DOCKER_APP_NAME=apc # DOCKER_APP_NAME에는 프로젝트명을 적는다.
...
if [ -n "$EXIST_AFTER" ]; then
    ...
    sudo cp ${SOURCE_REPOSITORY}/frontend/nginx/service-url-${AFTER_COMPOSE_COLOR}.inc /etc/nginx/conf.d/frontend-url.inc
    sudo cp ${SOURCE_REPOSITORY}/backend/nginx/service-url-${AFTER_COMPOSE_COLOR}.inc /etc/nginx/conf.d/backend-url.inc
